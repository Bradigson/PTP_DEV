using System;
using System.Collections.Generic;
using System.IO;
using System.Linq;
using System.Threading.Tasks;
using System.Web;
using System.Web.Mvc;
using BussinessLayer.Services;
using DataLayer.Models;
using DataLayer.PDbContex;


namespace POS.Controllers
{
    public class ClientesController : Controller
    {
        private ClienteService _clienteService;

        private RegionService _regionService;
        private PaisService _PaisService;
        private MunicipioService _municipoService;
        private BussinessLayer.Services.Contracts.ProvinciasService _provinciasService;
        private PDbContext db;

        private long idEmpresa()
        {
            return long.Parse(Session["IDEmpresa"].ToString());

        }

        public ClientesController()
        {
            _clienteService= new ClienteService();
            _regionService = new RegionService();
            db = new PDbContext();
           
        }

        // GET: Clientes
        public async Task<ActionResult> Index()
        {
            
            return View(await  _clienteService.GetAll(idEmpresa()));
        }

        public async Task<ActionResult> CrearCliente()
        {
            

            var pais =  db.SC_PAIS001.OrderBy(x => x.NOMBRE_PAIS).ToList();
            ViewData["pais"] = pais;


            //var pais_nacio = db.SC_PAIS001.ToList();
            //ViewData["pais_nacio"] = pais_nacio;


            var id_tipo = db.Tipo_Identificacion.ToList();
            ViewData["id_tipo"] = id_tipo;
            //ViewBag.Cod_Tipo_Identificacion = new SelectList(db.Tipo_Identificacion, "Cod_Tipo_Identificacion", "Descripcion");




            return View();
        }

        [HttpPost]
        public async Task<ActionResult> CrearCliente(Cliente cliente, HttpPostedFileBase Imagen)
        {
            
            if (ModelState.IsValid)
            {
                //"@TempData["titulo"]",
                //    text: "@TempData["message"]",
                @TempData["alerta"] = "x";
                try { 

                if (Imagen != null)
                {
                    //var url = System.IO.Path.GetFileName(Imagen.FileName);
                    //var fullUrl = System.IO.Path.Combine(Server.MapPath("~/Imagenes/Clientes"), url);
                    //Imagen.SaveAs(fullUrl);
                    //cliente.Imagen = "/Imagenes/Clientes/" + url;

                    string theFileName = Path.GetFileName(Imagen.FileName);
                    byte[] thePictureAsBytes = new byte[Imagen.ContentLength];
                    using (BinaryReader theReader = new BinaryReader(Imagen.InputStream))
                    {
                        thePictureAsBytes = theReader.ReadBytes(Imagen.ContentLength);
                    }
                    var extencioImg = System.IO.Path.GetExtension(Imagen.FileName);
                    cliente.Imagen = "data:image/"+ extencioImg.Replace('.', ' ').TrimStart() + ";base64," + Convert.ToBase64String(thePictureAsBytes);

                    //Imagenes\ClientesClienteDefault


                }
                else {
                    cliente.Imagen = @"data:image/jpg;base64,";


                }
                cliente.FechaCreacion = DateTime.Now;
                cliente.Fecha_Modificacion =  DateTime.Now;
                cliente.Usuario_Adicion =int.Parse(Session["ID"].ToString());
                cliente.Usuario_Modificacion = int.Parse(Session["ID"].ToString());
                cliente.IdEmpresa = idEmpresa();  
                await _clienteService.Add(cliente);

                    @TempData["titulo"] = "Guardado";
                    @TempData["message"] = "Cliente Guardado Exitosamente!";
                    @TempData["alerta"] = "success";

                    
                    return RedirectToAction("Index");


                }
                catch(Exception e)
                {

                    @TempData["message"] = "Cliente Guardado Exitosamente! "+e.Message;
                    @TempData["alerta"] = "warning";
                    return View(cliente);
                }
            }else
            {
                var lst = await _regionService.GetAll(idEmpresa());
                ViewData["ListaRegion"] = lst;
                @TempData["titulo"] = "Advertencia";
                @TempData["message"] = "Revisar la informacion! ";
                @TempData["alerta"] = "warning";
                return View(cliente);
            }
           
        }

        public async Task<ActionResult> EditarCliente(int id)
        {

            db = new PDbContext();
            var model = await _clienteService.GetById(id, idEmpresa());

            var pais = db.SC_PAIS001.ToList();
            ViewData["pais"] = pais;

            var reg = db.SC_REG001.Where(x=>x.CODIGO_PAIS==model.Cod_Pais).ToList();
            ViewData["reg"] = reg;

            var provin = db.SC_PROV001.Where(x => x.CODIGO_REG == model.Cod_Region).ToList();
            ViewData["provin"] = provin;

            var mun = db.SC_MUNIC001.Where(x => x.CODIGO_PROV == model.Cod_Provincia).ToList();
            ViewData["mun"] = mun;

            var cui_pais = db.Ciudades_X_Paises.ToList();
            ViewData["cui_pais"] = cui_pais;

            var pais_nacio = db.SC_PAIS001.ToList();
            ViewData["pais_nacio"] = pais_nacio;


            var id_tipo = db.Tipo_Identificacion.ToList();
            ViewData["id_tipo"] = id_tipo;

            return View(model);
        }


        public async Task<ActionResult> EditarCliente2(Cliente cliente, HttpPostedFileBase Imagen)
        {

            if (ModelState.IsValid)
            {
                if (Imagen != null)
                {
                    

                    string theFileName = Path.GetFileName(Imagen.FileName);
                    byte[] thePictureAsBytes = new byte[Imagen.ContentLength];
                    using (BinaryReader theReader = new BinaryReader(Imagen.InputStream))
                    {
                        thePictureAsBytes = theReader.ReadBytes(Imagen.ContentLength);
                    }
                    var extencioImg = System.IO.Path.GetExtension(Imagen.FileName);
                    cliente.Imagen = "data:image/" + extencioImg.Replace('.', ' ').TrimStart() + ";base64," + Convert.ToBase64String(thePictureAsBytes);
                    


                }
                    else
              {

                    cliente.Imagen = Session["imagen2"].ToString();

                    
                }
                cliente.Fecha_Modificacion = DateTime.Now;
                cliente.Usuario_Adicion = int.Parse(Session["ID"].ToString());
                cliente.IdEmpresa = idEmpresa();
                await _clienteService.Edit(cliente);

                return RedirectToAction("Index");
            }
            return View(cliente);
        }

        public async Task<ActionResult> InformacionCliente(int idCliente)
        {
            Cliente c = await _clienteService.GetById(idCliente, idEmpresa());
            return View(c);
        }

        public async Task<ActionResult> EliminarCliente(int idCliente)
        {
           
            await _clienteService.Delete(idCliente, idEmpresa());
            return RedirectToAction("Index");
        }
        public ActionResult ImprimirCliente(int id)
        {
            return RedirectToAction("Index");
        }
        private string GetUserIP()
        {
            string ipList = Request.ServerVariables["HTTP_X_FORWARDED_FOR"];

            if (!string.IsNullOrEmpty(ipList))
            {
                return ipList.Split(',')[0];
            }

            return Request.ServerVariables["REMOTE_ADDR"];
        }


    }
}