using System;
using System.Collections.Generic;
using System.Data;
using System.Data.Entity;
using System.Linq;
using System.Threading.Tasks;
using System.Net;
using System.Web;
using System.Web.Mvc;
using DataLayer.Models;
using DataLayer.PDbContex;
using BussinessLayer.Services;
using System.IO;

namespace POS.Controllers
{
    public class SC_EMP001Controller : Controller
    {
        private PDbContext db = new PDbContext();
        private SC_EMP001service _EMP001Service = new SC_EMP001service();

        
        // GET: SC_EMP001
        public async Task<ActionResult> Index()
        {
            return View(await db.SC_EMP001.ToListAsync());
        }

        // GET: SC_EMP001/Details/5
        public async Task<ActionResult> Details(int? id)
        {
            if (id == null)
            {
                return new HttpStatusCodeResult(HttpStatusCode.BadRequest);
            }
            SC_EMP001 sC_EMP001 = await db.SC_EMP001.FindAsync(id);
            if (sC_EMP001 == null)
            {
                return HttpNotFound();
            }
            return View(sC_EMP001);
        }

        // GET: SC_EMP001/Create
        public ActionResult Create()
        {
            return View();
        }

        // POST: SC_EMP001/Create
        // To protect from overposting attacks, please enable the specific properties you want to bind to, for 
        // more details see https://go.microsoft.com/fwlink/?LinkId=317598.
        [HttpPost]
        //[ValidateAntiForgeryToken]
        public async Task<ActionResult> Create([Bind(Include = "CODIGO_EMP,NOMBRE_EMP,LOGO_EMP,RNC_EMP,DIRECCION,TELEFONO1,TELEFONO2,EXT_TEL1,EXT_TEL2,CANT_SUCURSALES,CANT_USUARIO,WEB,USUARIO_ADICCION,FECHA_ADICION,USUARIO_MODIFICACION,FECHA_MODIFICACION,IDEmpresa")] SC_EMP001 sC_EMP001, HttpPostedFileBase Imagen)
        {
            if (ModelState.IsValid)
            {

                if (Imagen != null)
                {
                  

                    string theFileName = Path.GetFileName(Imagen.FileName);
                    byte[] thePictureAsBytes = new byte[Imagen.ContentLength];
                    using (BinaryReader theReader = new BinaryReader(Imagen.InputStream))
                    {
                        thePictureAsBytes = theReader.ReadBytes(Imagen.ContentLength);
                    }
                    var extencioImg = System.IO.Path.GetExtension(Imagen.FileName);
                    sC_EMP001.LOGO_EMP = "data:image/" + extencioImg.Replace('.', ' ').TrimStart() + ";base64," + Convert.ToBase64String(thePictureAsBytes);

                  


                }
                else
                {
                    sC_EMP001.LOGO_EMP = @"data:image/jpg;base64,";


                }
                sC_EMP001.FECHA_ADICION =DateTime.Now;
                sC_EMP001.FECHA_MODIFICACION = DateTime.Now;
                sC_EMP001.USUARIO_ADICCION =int.Parse( Session["ID"].ToString());
                sC_EMP001.USUARIO_MODIFICACION = int.Parse(Session["ID"].ToString());
               
                
                db.SC_EMP001.Add(sC_EMP001);
                await db.SaveChangesAsync();
                return RedirectToAction("Index");
            }

            return View(sC_EMP001);
        }

        // GET: SC_EMP001/Edit/5
        public async Task<ActionResult> Edit(int? id)
        {
            if (id == null)
            {
                return new HttpStatusCodeResult(HttpStatusCode.BadRequest);
            }
            SC_EMP001 sC_EMP001 = await db.SC_EMP001.FindAsync(id);
            if (sC_EMP001 == null)
            {
                return HttpNotFound();
            }
            return View(sC_EMP001);
        }

        // POST: SC_EMP001/Edit/5
        // To protect from overposting attacks, please enable the specific properties you want to bind to, for 
        // more details see https://go.microsoft.com/fwlink/?LinkId=317598.
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<ActionResult> Edit([Bind(Include = "CODIGO_EMP,NOMBRE_EMP,LOGO_EMP,RNC_EMP,DIRECCION,TELEFONO1,TELEFONO2,EXT_TEL1,EXT_TEL2,CANT_SUCURSALES,CANT_USUARIO,WEB,USUARIO_ADICCION,FECHA_ADICION,USUARIO_MODIFICACION,FECHA_MODIFICACION")] SC_EMP001 sC_EMP001, HttpPostedFileBase Imagen)
        {
            if (ModelState.IsValid)
            {

                if (Imagen != null)
                {


                    string theFileName = Path.GetFileName(Imagen.FileName);
                    byte[] thePictureAsBytes = new byte[Imagen.ContentLength];
                    using (BinaryReader theReader = new BinaryReader(Imagen.InputStream))
                    {
                        thePictureAsBytes = theReader.ReadBytes(Imagen.ContentLength);
                    }
                    var extencioImg = System.IO.Path.GetExtension(Imagen.FileName);
                    sC_EMP001.LOGO_EMP = "data:image/" + extencioImg.Replace('.', ' ').TrimStart() + ";base64," + Convert.ToBase64String(thePictureAsBytes);




                }

                db.Entry(sC_EMP001).State = EntityState.Modified;
                await db.SaveChangesAsync();
                return RedirectToAction("Index");
            }
            return View(sC_EMP001);
        }

        // GET: SC_EMP001/Delete/5
        public async Task<ActionResult> Delete(int? id)
        {
            SC_EMP001 sC_EMP001 = await db.SC_EMP001.FindAsync(id);
            db.SC_EMP001.Remove(sC_EMP001);
            await db.SaveChangesAsync();
            return RedirectToAction("Index");
        }

     
        protected override void Dispose(bool disposing)
        {
            if (disposing)
            {
                db.Dispose();
            }
            base.Dispose(disposing);
        }
        
    }
}
